...
/**
 * The same reading, with blank and rubbish both collapsed to zero.
 *
 * The distinction the rest of this module works so hard to keep is exactly what
 * a live calculation doesn't want. A screen that recomputes on every keystroke
 * has to have an answer for a field holding "1." or nothing at all, and the
 * answer that keeps the figures on screen steady is zero — the alternative is a
 * readout that blanks out between two keypresses.
 *
 * Only for display maths. Anything that saves should use `parseDecimal` and
 * keep the three outcomes apart.
 *
 * @param {string|number|null|undefined} input
 * @returns {number}
 */
export function parseDecimalOrZero(input) {
  const value = parseDecimal(input);
