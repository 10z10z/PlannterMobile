...
/** The events recorded in a date range, for one side of the app. */
export async function fetchEvents({ from, to, scope }) {
  let query = supabase
    .from('activity_events')
  ...
  query = scope === 'station' ? query.not('station_id', 'is', null) : query.not('growspace_id', 'is', null);
...
async function derivedStationEntries({ from, to }) {
  const [{ data: sowings }, { data: cells }] = await Promise.all([
    supabase
      .from('sowings')
      .select('id, station_id, seed_pack_name, sown_on, grid_rows, grid_cols, station:germination_stations(name)')
      .gte('sown_on', from)
      .lte('sown_on', to),
    supabase
      .from('sowing_cells')
  ...
  const entries = (sowings ?? []).map((sowing) => ({
    ...
    sowing_id: sowing.id,
    plant_id: null,
    place: sowing.station?.name ?? null,
    derived: true,
  }));
...
async function derivedGrowspaceEntries({ from, to }) {
  ...
  const entries = (planted ?? []).map((plant) => ({
    ...
    sowing_id: null,
    plant_id: plant.id,
    place: plant.growspace?.name ?? null,
    derived: true,
  }));
...
/**
 * Everything that belongs on one calendar for one month: what was recorded,
 * plus whatever the data itself remembers and the log doesn't already say.
 */
export async function fetchActivity({ from, to, scope }) {
  const [recorded, derived] = await Promise.all([
    fetchEvents({ from, to, scope }),
    scope === 'station'
      ? derivedStationEntries({ from, to })
      : derivedGrowspaceEntries({ from, to }),
  ]);

  return mergeEntries(recorded, derived);
}
