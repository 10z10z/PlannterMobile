...
export function photoperiodSummary(lights) {
  const cycles = [
    ...new Set((lights ?? []).map((row) => photoperiodLabel(row.hours_on)).filter(Boolean)),
  ];
...
/**
 * A section that can't be read falls back to empty instead of taking the rest
 * of the page down with it.
 *
 * The three sections are independent — they share a screen, not a query — so
 * one of them failing is no reason to show a grower a blank dashboard with
 * their growspaces still sitting there in the database. Which is exactly what
 * `Promise.all` did the first time this shipped.
 */
 ...
 *
 * The three sections are fetched together rather than one after another, so the
 * screen arrives whole instead of filling in around the reader.
 */
export async function fetchDashboard({
  ...
  const [actions, growspaces, activity] = await Promise.all([
    settled(fetchOpenActions({ today, soonDays }), []),
    settled(fetchGrowspaceOverview(), []),
    settled(fetchActivity({ from: shiftDate(today, -recentDays), to: today }), []),
  ]);
