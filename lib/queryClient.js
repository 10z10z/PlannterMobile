...
import { endExpiredSession } from './sessionGuard';
...
/**
 * Every failure in the app passes through here on its way to a screen, which is
 * the one place guaranteed to see the raw error before `messageFor` replaces it
 * with a sentence. Two things happen with it.
 *
 * It is logged, in development only: a released build has nowhere useful to put
 * this, and shipping it would only slow the device down.
 *
 * And it is checked for a session the server has stopped accepting, which is
 * the one failure the app can do something about on the grower's behalf rather
 * than merely describe. See `./sessionGuard`.
 */
