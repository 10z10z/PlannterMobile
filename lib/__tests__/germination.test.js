...
describe('daysSince', () => {
  // Stored dates are local days, so the fixtures are built from local parts.
  // `toISOString` reads the UTC day, which is a different day for part of every
  // evening east of Greenwich and made this pass only some of the time.
  const iso = (date) =>
  ...
  it('never counts backwards for a date in the future', () => {
    expect(daysSince(iso(new Date(Date.now() + 86400000)))).toBe(0);
  });
