...
jest.mock('../activity', () => ({ fetchActivity: jest.fn() }));
...
describe('fetchDashboard', () => {
  ...
  it('puts the three sections together', async () => {
    ...
    fetchActivity.mockResolvedValue([{ id: 'e1', occurred_on: '2026-08-05' }]);
  ...
  it('still shows the plans when the log cannot be read', async () => {
    ...
    fetchActivity.mockRejectedValue(new Error('offline'));
  ...
  it('comes back empty rather than throwing when nothing can be read', async () => {
    ...
    fetchActivity.mockRejectedValue(new Error('down'));
