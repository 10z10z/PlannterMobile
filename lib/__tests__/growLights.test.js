...
describe('lightSpecs', () => {
  it('takes the most useful figures first', () => {
    expect(lightSpecs(panel)).toEqual(['150 W', '3500K', 'Full spectrum', '600 PPFD @ 45cm']);
  });
...
describe('assignmentSummary', () => {
  ...
  it('drops the cycle when none has been set', () => {
    expect(assignmentSummary({ quantity: 1, light: panel })).toBe('150 W · 3500K · Full spectrum');
  });

  it('is the cycle alone for a fixture with no published figures', () => {
    expect(assignmentSummary({ quantity: 1, hours_on: 16, light: { type: 'led' } })).toBe('16/8 h');
  });
...
describe('parseHours', () => {
  it('reads a typed cycle as a number', () => {
    expect(parseHours('18')).toBe(18);
    expect(parseHours('18.5')).toBe(18.5);
  });
  ...
  it('keeps a cycle of nothing rather than reading it as unset', () => {
    expect(parseHours('0')).toBe(0);
  });

  it('is null for an empty or unfinished field', () => {
