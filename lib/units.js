...
/**
 * The number a `parse*` should convert, or `null` if there isn't one.
 *
 * These are conversion functions, not validators: by the time input reaches one
 * the form has already had its say, so anything unparseable is treated the same
 * as an empty field rather than thrown. What this adds over a bare `Number()`
 * is `1,5`, and the refusal to read `1e3` as a thousand.
 *
