...
/**
 * Grow lights plus how many of each group are hanging in a growspace. The count
 * is summed from the assignment rows rather than stored, so it can't drift out
 * of sync with where the lights actually are.
 */
export async function fetchGrowLightsWithUsage() {
  const [{ data: lights, error }, { data: assigned }] = await Promise.all([
