...
import { SCHEDULE_KINDS } from './enums';
...
// Defined in ./enums, with the note on what `targets` and `whole` mean, so
// validation can read it without loading the client.
 ...
 */
export async function fetchScheduled({ from, to, scope = null }) {
  let query = supabase
 ...
 * reminder either way.
 */
export async function saveScheduledAction({ userId = null, id, targets, ...fields }) {
  ...
  const query = id
    ...
    : supabase.from('scheduled_actions').insert({ ...payload, user_id: owner });

  const { data, error } = await query.select().single();
  if (error) throw error;

  await saveTargets({ userId: owner, actionId: data.id, targets });
